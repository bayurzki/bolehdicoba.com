...
!function ($) {
    ...
    var DateRangePickerCustom = function (element, options, cb) {
        ...
        var DRPTemplate = '<div class="daterangepickercustom dropdown-menu">' +
                '<div class="ranges">' +
                '</div>' +
                clist +
                '<div class="controls">' + 
                    '<span style="float:left;margin-left: 12px;">Compare : </span><br>'+
                    '<ul class="range-types uk-margin-small-bottom uk-padding-remove">' +
                        '<li id="singleRange" class="active">Single Range</li>' +
                        '<li id="multiSameRange">Compare Previous</li>' +
                        '<li id="multiDiffRange">Compare Custom</li>' +
                    '</ul>' + 
                    '<button class="btn btn-small btn-default clearBtn" style="margin-left: 8px;">' + this.locale.clearLabel + '</button>' +
                    '<button class="btn btn-small btn-primary applyBtn" disabled="disabled" style="margin-left: 8px;">' + this.locale.applyLabel + '</button>&nbsp;' +
                '</div>' +
              '</div>';
    ...
    DateRangePickerCustom.prototype = {

        constructor: DateRangePickerCustom,
        ...
        clickRange: function (e) {
            ...
            for (var range in this.ranges) {
                ...
                if (this.startDate == this.ranges[range][0] && this.endDate == this.ranges[range][1]) {
                    customRange = false;
        ...
        renderCalendar: function (calendar, cnum) {
            ...
            if (cnum == 0 && (!this.minDate || this.minDate < calendar[1][1])) {
                ...
                html += '<th class="prev available"><span class="mdi mdi-arrow-left-bold-circle mdi-18px bdd-color-3"></span></th>';
            }
            else {
                html += '<th></th>';
            }
            ...
            if (cnum == (this.calendar_num - 1) && (!this.maxDate || this.maxDate > calendar[1][1])) {
                html += '<th class="next available"><span class="mdi mdi-arrow-right-bold-circle mdi-18px bdd-color-3"></span></th>';
            }
            else {
                 html += '<th></th>';
            }
    ...
    $.fn.daterangepickercustom = function (options, cb) {
      this.each(function() {
        var el = $(this);
        if (!el.data('daterangepickercustom'))
          el.data('daterangepickercustom', new DateRangePickerCustom(el, options, cb));
      });
      return this;
    };
