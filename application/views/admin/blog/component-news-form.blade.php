...
@endphp
@if ($cond)
    <div id="component-form-app" class="uk-background-default" style="padding-top: 20px !important" x-data="initState('{{ $data[0]->style_id }}')">    
@else
    <div id="component-form-app" class="uk-background-default" style="padding-top: 20px !important" x-data="initState('1')">    
@endif
    @foreach ($data as $row)        
	<form id="content_form"
		action="{{ !empty(@$row) ? base_url('post/editNewsPosts/' . $row->id) : base_url('post/addNewsPosts') }}"
        method="POST" 
        ...
        <div class="main-title" style="z-index: 980;" uk-sticky="bottom: #offset;animation: uk-animation-slide-bottom-small">
            <div class="uk-card uk-card-default uk-card-body" style="padding: 10px 20px">
                <div class="uk-float-left">
                	<h3 class="uk-text-muted">{{ !empty(@$row) ? 'Edit News Posts' : 'Add News Posts'}}</h3>
                </div>
		...
		<div class="uk-animation-slide-bottom-small" x-show="toggleForm">
            ...
            <input type="hidden" name="news_id" value="{{ $CI->session->userdata()['news_id'] }}">
				...
				<div class="uk-width-1-1@s">
					...
					<select id="style" class="uk-select" name="style_id" @change="setStyle" required>
                        ...
                        @if ($style_row->name != 'image-with-right-content' && $style_row->name != 'clients-says' && $style_row->name != 'comments-section' && $style_row->name != 'carousel') {
                            <option value="{{ $style_row->id }}" {{ @$style_row->id == @$row->style_id ? 'selected' : '' }}>
                                {{ $style_row->name }}
                            </option>
                        @endif
                        @endforeach
				...
				<div class="uk-grid-margin uk-width-1-2@s" x-show="!title">
                    ...
                    <input id="title" class="onchange uk-input" type="text" placeholder="{{ $title }}" name="title"
				...
				<div class="uk-grid-margin uk-width-1-2@s" x-show="!title">
                    ...
                    <input id="sub_title" class="onchange uk-input" type="text" placeholder="{{ $sub_title }}" name="sub_title"
						value="{{ !empty(@$row) ? $row->sub_title : '' }}">
				</div>      
				...
				<div class="uk-grid-margin uk-width-1-1@s" x-show="!description">
					...
					<textarea id="description" class="onchange uk-input" placeholder="{{ $description }}" name="description">
                        {{ !empty(@$row) ? $row->description : '' }}
                    </textarea>
				</div>    
                ...
                <div class="uk-width-1-2@s" x-show="!image">
                    ...
                    <p class="file-output uk-margin-medium-left">{{ !empty(@$row->img_path) ? $row->img_path : ''}}</p>
                </div>
				...
				<div class="uk-width-1-2@s image-preview-container" x-show="!image">
                    ...
                    <div class="uk-flex uk-flex-wrap gallery-container">
                        ...
                        <img 
                            ...
                            id="image-preview"
                            src="@php echo base_url() . 'assets/images/gallery/' . $gallery_row->img_path @endphp"                     
                            alt="image_preview" />                                                    
                        @endforeach
                    </div>                     
                    @else
